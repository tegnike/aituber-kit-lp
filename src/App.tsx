  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 h-[72px] flex items-center justify-between">
        ...
        <nav className={`hidden md:flex items-center gap-10 text-[15px] font-semibold tracking-wide transition-colors duration-500 ${scrolled ? 'text-ink-soft' : 'text-white/90'}`}>
          <a href="#usecases" className="hover:text-purple transition-colors">
            活用シーン
          </a>
          <a href="#characters" className="hover:text-purple transition-colors">
            キャラクター
          </a>
          <a href="#specs" className="hover:text-purple transition-colors">
            AI連携
          </a>
          <a href="#setup" className="hover:text-purple transition-colors">
            セットアップ
          </a>
        </nav>
        ...
        <a
          ...
          className="hidden md:inline-flex items-center gap-2 px-6 py-3 text-[15px] font-bold tracking-wide bg-ink text-white rounded-full hover:bg-purple-deep transition-colors"
        >
          <svg
        ...
        >
          <div className="space-y-1.5">
            <div
              className={`w-5 h-0.5 transition-all ${scrolled ? 'bg-ink' : 'bg-white'} ${mobileOpen ? 'rotate-45 translate-y-2' : ''}`}
            />
            <div
              className={`w-5 h-0.5 transition-all ${scrolled ? 'bg-ink' : 'bg-white'} ${mobileOpen ? 'opacity-0' : ''}`}
            />
            <div
              className={`w-5 h-0.5 transition-all ${scrolled ? 'bg-ink' : 'bg-white'} ${mobileOpen ? '-rotate-45 -translate-y-2' : ''}`}
            />
        ...
        <div className="max-w-[780px]">
          ...
          <div className="reveal reveal-delay-3 mt-8 md:mt-10 flex flex-wrap gap-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2.5 px-8 py-4 bg-white text-ink text-[15px] font-bold rounded-full hover:bg-purple hover:text-white transition-all hover:-translate-y-0.5 shadow-2xl"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center px-8 py-4 text-[15px] font-bold text-white/90 rounded-full border-2 border-white/20 hover:border-white/50 transition-all hover:-translate-y-0.5 backdrop-blur-sm"
            >
              ドキュメントを見る
            </a>
          </div>
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {CASES.map((c, i) => (
            <div
              key={c.label}
              className={`reveal ${i > 0 ? `reveal-delay-${Math.min(i, 3)}` : ''} group`}
            >
              <div className="rounded-2xl overflow-hidden bg-white border border-ink/5 hover:shadow-xl hover:-translate-y-1 transition-all duration-500">
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6">
          {MODELS.map((m, i) => (
            <div
              key={m.name}
              className={`reveal reveal-delay-${i + 1} group bg-white rounded-3xl overflow-hidden border border-ink/5 hover:shadow-xl hover:-translate-y-1 transition-all duration-500`}
            >
        ...
        <div className="reveal reveal-delay-1 mb-10">
          ...
          <div className="flex flex-wrap gap-2">
            {AI_PROVIDERS.map((p) => (
              <span
                key={p}
                className="px-4 py-2 rounded-full text-[13px] font-medium bg-purple/5 text-purple border border-purple/8 hover:bg-purple/10 transition-colors"
              >
                {p}
              </span>
            ))}
        ...
        <div className="reveal reveal-delay-2">
          ...
          <div className="flex flex-wrap gap-2">
            {TTS_ENGINES.map((e) => (
              <span
                key={e}
                className="px-4 py-2 rounded-full text-[13px] font-medium bg-green/5 text-green border border-green/8 hover:bg-green/10 transition-colors"
              >
                {e}
              </span>
            ))}
      ...
      <div className="relative z-10 text-center max-w-[640px] mx-auto">
        ...
        <div className="reveal reveal-delay-3 mt-10 flex flex-col sm:flex-row gap-4 justify-center">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center justify-center gap-2.5 px-10 py-5 bg-white text-ink text-[16px] font-bold rounded-full hover:bg-purple hover:text-white transition-all hover:-translate-y-1 shadow-2xl"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
            </svg>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center justify-center px-10 py-5 text-[16px] font-bold text-white/80 rounded-full border-2 border-white/15 hover:border-white/40 transition-all hover:-translate-y-1"
          >
            商用ライセンスについて
          </a>
        </div>
